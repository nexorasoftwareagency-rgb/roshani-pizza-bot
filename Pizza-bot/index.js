...
async function startBot() {
    ...
    // The command listener is initialized at the top via initCommandListener(sock)
    // to ensure it handles all command types (Weekly, Monthly, Daily) and cleans up the DB.
    ...
    sock.ev.on('messages.upsert', async (m) => {
        try {
            ...
            if (user.step === "CATEGORY") {
                ...
                user.dishList = Object.entries(dishes)
                    ...
                    .map(([id, d]) => {
                        ...
                        const stockCount = invItem ? (invItem.stock || 0) : 999; // Default to 999 if not in inventory
            ...
            if (user.step === "DISH") {
                ...
                // FINAL STOCK CHECK before allowing selection
                if (!dish.inStock) {
                    ...
                    // Notify Admin of interest in out-of-stock item
