    ...
    onValue(notifQuery, snap => {
        ...
        notifications.reverse().forEach(n => {
            ...
            div.addEventListener('click', () => {
                ...
                update(ref(db, updatePath), { read: true });
            });
...
window.clearAllNotifications = async () => {
    ...
    if (await showConfirmModal("CLEAR NOTIFICATIONS", "Clear all notifications?")) {
        remove(ref(db, `riders/${currentUser.profile.id}/notifications`));
    }
};
...
window.toggleRiderStatus = async () => {
    ...
    try {
        await update(ref(db, resolvePath(`riders/${currentUser.profile.id}`)), { status: newStatus });
...
window.verifyOTP = async () => {
    ...
    const userAttempts = attempts[currentOrderId] || { count: 0, lastTry: 0, blockedUntil: 0 };

    if (userAttempts.blockedUntil > now) {
        const remaining = Math.ceil((userAttempts.blockedUntil - now) / 1000);
        showToast(`Too many attempts! Blocked for ${remaining}s`, "error");
        return;
    }
    ...
    try {
        ...
        if (matchesCustomer || matchesFallback) {
            ...
            delete attempts[currentOrderId];
            ...
            if (userAttempts.count >= 5) {
                userAttempts.blockedUntil = now + (5 * 60 * 1000); // 5 min block
                showToast("5 failed attempts! Blocked for 5 minutes.", "error");
            } else {
                showToast(`Code mismatch! ${5 - userAttempts.count} attempts remaining.`, "error");
            }
            attempts[currentOrderId] = userAttempts;
            localStorage.setItem('otp_attempts', JSON.stringify(attempts));
        }
    } catch (e) {
...
window.regenerateOTP = async () => {
    ...
    // Rate Limiting Check
    const now = Date.now();
    const resends = JSON.parse(localStorage.getItem('otp_resends') || '{}');
    const userResends = resends[currentOrderId] || { count: 0, lastResend: 0 };

    if (now - userResends.lastResend < 60000) { // 60s cooldown
        const remaining = Math.ceil((60000 - (now - userResends.lastResend)) / 1000);
    ...
    try {
        ...
        await update(ref(db, orderPath), { deliveryOTP: newOTP });
        
        // Update resend state
        userResends.count++;
        userResends.lastResend = now;
        resends[currentOrderId] = userResends;
        localStorage.setItem('otp_resends', JSON.stringify(resends));
