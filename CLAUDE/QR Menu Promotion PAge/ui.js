...
export function renderPromotionsLinks(store) {
    const wrap = document.getElementById('promoSocialSection');
    if (!wrap) return;
    const s = store || {};
    ...
    const label = `<div class="promo-section-label">💬 Follow &amp; Connect</div>`;

    if (links.length === 0) {
        wrap.innerHTML = `${label}<div class="promotions-empty">No social links have been set up yet. Check back soon!</div>`;
        return;
    }
    ...
    wrap.innerHTML = `${label}<div class="promotions-links">${cards}</div>`;
}

/**
 * Active Offers — live coupon-code discounts, sourced from the same
 * `discounts` node the Admin Discounts tab manages. Only `type:"coupon"`
 * discounts are shown here, because that's the only discount type the
 * QR checkout (discount.js's validateCoupon) actually honors — showing
 * a "global" or "first order" discount here would promise something
 * checkout can't currently apply automatically.
 *
 * `onUseCode(code)` is called when the customer taps "Use Code" — the
 * caller (app.js) is responsible for jumping to the Cart screen,
 * filling the code into the input, and applying it.
 */
export function renderActiveOffers(discounts, onUseCode) {
    const wrap = document.getElementById('activeOffersSection');
    if (!wrap) return;

    const now = Date.now();
    const offers = Object.entries(discounts || {})
        .map(([id, d]) => ({ id, ...d }))
        .filter(d =>
            d && d.type === 'coupon' && d.couponCode
            && d.enabled !== false
            && now >= (d.startsAt || 0)
    ...
    const label = `<div class="promo-section-label">🎉 Active Offers</div>`;

    if (offers.length === 0) {
        wrap.innerHTML = `${label}<div class="offers-empty">No active offers right now — check back soon!</div>`;
        return;
    }

    const cards = offers.map(d => {
        const isPercent = d.mode === 'percent';
        const conditionParts = [];
        if (d.minSubtotal) conditionParts.push(`On orders above ${fmtMoney(d.minSubtotal)}`);
        if (isPercent && d.maxCap) conditionParts.push(`up to ${fmtMoney(d.maxCap)} off`);
        const condition = conditionParts.join(' · ') || 'Applies at checkout';

        return `
        <div class="offer-card">
            <div class="offer-value-strip">
                <span class="offer-value-num">${isPercent ? Math.round(d.value) + '%' : fmtMoney(d.value)}</span>
                <span class="offer-value-unit">OFF</span>
            </div>
            <div class="offer-details">
                <div class="offer-name">${esc(d.name || 'Special Offer')}</div>
                <div class="offer-condition">${esc(condition)}</div>
                <div class="offer-code-row">
                    <span class="offer-code-chip">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                        ${esc(d.couponCode)}
                    </span>
                    <button class="offer-use-btn" data-code="${esc(d.couponCode)}">Use Code</button>
                </div>
            </div>
        </div>`;
    }).join('');

    wrap.innerHTML = `${label}${cards}`;

    wrap.querySelectorAll('.offer-use-btn').forEach(btn => {
        btn.addEventListener('click', () => onUseCode && onUseCode(btn.dataset.code));
    });
}
}
