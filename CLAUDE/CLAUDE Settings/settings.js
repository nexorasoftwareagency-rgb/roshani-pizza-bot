...
function validateOptionalUrl(url, label) {
    if (!url) return true;
    try {
        // Accept protocol-relative / bare-domain entries too, just sanity check it's not garbage text.
...
export async function loadStoreSettings() {
    ...
    try {
        const [storeSnap, delSnap, botSnap, dispSnap, dineSnap] = await Promise.all([
        ...
        checks.forEach(id => setChecked(id, vi[id] !== false)); // default to true
...
export async function saveStoreSettings() {
    ...
    try {
        ...
        const coordsEl = document.getElementById('displayCoords');
        if (coordsEl) coordsEl.innerText = `${lat}, ${lng}`;
...
function getSlabsFromTable() {
    ...
    document.querySelectorAll('#feeSlabsTable tr').forEach(tr => {
        const kmEl = tr.querySelector('.slab-km');
        const feeEl = tr.querySelector('.slab-fee');
        if (!kmEl) return;
...
document.addEventListener('change', (e) => {
    ...
    if (e.target.id && e.target.id.startsWith('botImg')) {
...
document.addEventListener('click', (e) => {
    ...
    if (e.target.closest && e.target.closest('[data-action="removeFeeSlab"]')) {
