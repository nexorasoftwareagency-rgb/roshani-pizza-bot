...
function _orderListRow(o) {
    ...
    const isNew = (_nowMs() - (o.createdAt || 0)) < 120000;
    return `
...
function _elapsedLabel(createdAt) {
    const diff = Math.max(0, _nowMs() - (createdAt || _nowMs()));
...
function _kdsCard(o) {
    ...
    const mins = Math.floor((_nowMs() - (o.createdAt || _nowMs())) / 60000);
    ...
    <div class="kds-card ${urgentCls}" data-order-id="${_esc(o.id)}">
        ...
        <div class="kds-card-footer">
            <span class="kds-elapsed" data-created-at="${o.createdAt || _nowMs()}">${_elapsedLabel(o.createdAt)}</span>
...
async function _closeSessionForTable(tableId) {
    ...
    const sess = _sessionForTable(tableId);
    if (!t || !sess) return;
    const ok = await showConfirm(`Confirm payment received for Table ${t.number} (₹${Number(sess.grandTotal || sess.runningTotal || 0).toLocaleString('en-IN')})? This will close the session and free the table.`, 'Close Table');
    if (!ok) return;
    try {
        ...
        await runTransaction(Outlet.ref(`tableAnalytics/${tableId}`), (cur) => {
            ...
            cur.totalRevenue = (cur.totalRevenue || 0) + Number(sess.grandTotal || sess.runningTotal || 0);
        ...
        showToast('Table closed — bill settled', 'success');
        haptic(30);
    } catch (e) {
